						...
						int numInstrs)
{
  ...
  while(enteredCount < numInstrs || !isPipelineEmpty(pipeline)){
	...
	if(i == PIPE_WRITE_REG_STAGE){
	  ...
	  // the decode (read) stage and has a RAW dependence. If not, advance
	  // the instruction one stage. Otherwise, stall for one stage (do 
	  // nothing)
	  if (pipeline[i+1] == NULL) {  
	    if(i == PIPE_READ_REG_STAGE){
	      if(!hasRAWDependence(pipeline, i)) {
	        // advance the instruction
	        pipeline[i+1] = pipeline[i];
	        pipeline[i] = NULL;
	      } else {
	    	// Has RAW dependence, stall for one stage.
	      }
	    } else {
	      // advance the instruction
	      pipeline[i+1] = pipeline[i];
...
int hasRAWDependence(Instruction *pipeline[], int index){
  ...
  for(int i = 0; i < numSrcRegs; i++){
    ...
    // check every stage after the current stage
    for(int j = index + 1; j < NUM_STAGES; j++){
      if(pipeline[j] != NULL) {
	  ...
	  // check if source reg and dest reg are the same
	  if(srcReg == destReg){
